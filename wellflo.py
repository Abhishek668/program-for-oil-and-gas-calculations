...
def rate_dimensionless(rD, tD):
    ...
    if tD < (0.25 * rD**2):
        ...
        if tD > 0.01 and tD < 200:
          # Eq. 6.42
          qD = (26.7544 + (45.5537 * np.sqrt(tD)) + (13.3813 * tD) + (0.492949 * tD * np.sqrt(tD))
                ) / ((47.4210 * np.sqrt(tD)) + (35.5372 * tD) + (2.60967 * tD * np.sqrt(tD)))
        if tD >= 200:
          # Eq. 6.43
          qD = ((2.02623 * tD * (np.log(tD) - 1)) + \
                3.90086) / (tD * ((np.log(tD))**2))
...
def constant_terminal_rate(time, distance, re, rw, pi, q, poro, ct, k, h, mu_oil, Bo):
  ...
  Ei_table = Ei_table = np.loadtxt("Exponential%20Integral.txt") 
