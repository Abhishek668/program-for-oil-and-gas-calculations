...
def constant_rate_drawdown_test(t, p, q, Bo, mu_oil, h, poro, ct, rw, pi, your_guess=5):
    ...
    labels2 = []
    labels2.append("End Time of MTR = {} hours".format(
        np.round(t[your_guess], 3)))
...
def constant_rate_buildup_test(t, p, q, Bo, mu_oil, h, poro, ct, rw, t_since_shutin, your_guess=5):
    ...
    labels2 = []
    labels2.append(
        "Time @ shut-in = {} hours".format(np.round(t_since_shutin, 1)))
    labels2.append("End of ETR Time = {} hours".format(
        np.round(t[your_guess], 3)))
    handles2 = [mpl_patches.Rectangle((0, 0), 1, 1, fc="white", ec="white",
                                      lw=0, alpha=0)] * 2
