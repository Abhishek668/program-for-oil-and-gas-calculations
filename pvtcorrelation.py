...
def gas_compressibility(T_pr, P_pr, rho_pr, z, P_pc):
    ...
    c_pr_analytical = (1 / P_pr) - ((0.27 / (z**2 * T_pr))
                                    * (do / (1 + ((rho_pr / z) * do))))
...
def oil_pbubble(Rsb, sg2, api, temp2):
    ...
    if Rsb > 20 and Rsb < 2070 and sg2 > 0.56 and sg2 < 1.18 and api > 16 and api < 58 and temp2 > 70 and temp2 < 295:
        ...
        P_bubble = (
            Rsb / (c1 * sg2 * np.exp((c3 * api)/(temp2 + 459.67))))**(1 / c2)
    else:
        P_bubble = np.nan
    return P_bubble
...
def oil_fvf(P_bubble, api, Rsb, sg2, temp2, pressure2):
    ...
    * At and bubble-point pressure
    ...
    so = 141.5 / (api + 131.5)
    Bo_bubble = 1 + ((0.0005 * Rsb) * ((sg2 / so)**0.25)) + \
        ((0.0004*(temp2 - 60)) / (so * sg2))  # temp in def F
    ...
    if pressure2 < P_bubble:  # use Vazquez-Beggs
        ...
        if api > 30:
            ...
            c5 = 1.100E-5
            c6 = 1.337E-9
        Rsc = (pressure2**c2) * c1 * sg2 * \
            np.exp((c3 * api) / (temp2 + 459.67))
        Bo = 1 + (c4 * Rsc) + (c5 * (temp2 - 60) * (api / sg2)) + \
            (c6 * Rsc * (temp2 - 60) * (api / sg2))  # temp in deg F
    if pressure2 == P_bubble:
        # use Levitan-Murtha
        Bo = Bo_bubble
    if pressure2 > P_bubble:
        # Calculate oil compressibility first using Levitan-Murtha
        coil = ((5 * Rsb) + (17.2 * temp2) - (1180 * sg2) +
                (12.61 * api) - 1433) / (1E+05 * pressure2)
    ...
    * Above bubble-point pressure
    ...
    if pressure2 > P_bubble:
        ...
        if pressure2 > 126 and pressure2 < 9500 and sg2 > 0.511 and sg2 < 1.351 and api > 15.3 and api < 59.5 and Rs > 9.3 and Rs < 2199:
            ...
            m = 2.6 * (pressure2**1.187) * \
                np.exp(-11.513 - (8.98E-05 * pressure2))
...
def oil_compressibility(pressure2, P_bubble, temp2, api, Rsb, sg2):
    ...
    * Above and at bubble-point pressure
    ...
    if pressure2 < P_bubble:
        # use McCain
        ln_coil = -7.573 - (1.45 * np.log(pressure2)) - (0.383 * np.log(P_bubble)) + (
            1.402 * np.log(temp2)) + (0.256 * np.log(api)) + (0.449 * np.log(Rsb))
        coil = np.exp(ln_coil)
    if pressure2 >= P_bubble:
        # use Vazquez-Beggs
        coil = ((5 * Rsb) + (17.2 * temp2) - (1180 * sg2) +
...
def water_fvf(temp, p):
    ...
    # temp in Fahrenheit
    # p pressure in psia
    Vwp = (-1.95301E-9 * p * temp) - (1.72834E-13 * (p**2) * temp) - \
        (3.588922E-7 * p) - (2.25341E-10 * p**2)
...
#   import gas_fvf, water_pbubble
    ...
    if p < pbubble:
        first_term = - \
            (1 / ((7.033 * p) + (0.5415 * s) - (537 * temp) + (403300)))

        # calculate Bg @ sg=0.63
        Bg = gas_fvf(0.63, temp, p) / 5.615  # convert res ft3/SCF to RB/SCF

        B = 1.01021E-2 - (7.44241E-5 * temp) + (3.05553E-7 *
                                                (temp**2)) - (2.94883E-10 * (temp**3))
        C = -1E-7 * (9.02505 - (0.13023 * temp) + (8.53425E-4 * (temp**2)
                                                   ) - (2.34122E-6 * (temp**3)) - (2.37049E-9 * (temp**4)))
